...
export const ColorRow = ({ label, value, onChange, onCommit, min, max, textScale, textStroke, highlightColor, onInteractionStart, onInteractionEnd, precision = 0, innerGap = 8, labelWidth = 16 }: any) => {
  ...
  return (
    <div className="flex items-center group/scrub w-full overflow-visible" style={{ gap: innerGap }} onMouseDown={e => e.stopPropagation()}>
      ...
      <input type="range" min={min} max={max} step={precision === 0 ? 1 : 0.01} value={value} onMouseDown={onInteractionStart} onChange={e => { const newVal = parseFloat((e.target as HTMLInputElement).value); onChange(newVal, newVal - value); }} onMouseUp={e => { onCommit(parseFloat((e.target as HTMLInputElement).value), false); onInteractionEnd?.(); }} style={{ accentColor: highlightColor }} className="flex-1 min-w-0" />
      {isEditing ? (
