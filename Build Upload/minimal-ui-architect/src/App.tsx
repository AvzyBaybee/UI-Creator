...
export default function App() {
  ...
  const elementsRef = useRef(elements); const [history, setHistory] = useState<CanvasElement[][]>([INITIAL_ELEMENTS]);
  const [historyIndex, setHistoryIndex] = useState(0); const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const selectedIdsRef = useRef(selectedIds); const [clipboard, setClipboard] = useState<CanvasElement[]>([]);
  const clipboardRef = useRef(clipboard); const [tool, setTool] = useState<Tool>('select');
      ...
      <div className="flex-1 relative bg-[#1a1a1a]">
        ...
        >
          <Layer>
            ...
            {selectedIds.length > 0 && tool === 'select' && elements.find(el => el.id === selectedIds[0])?.type === 'tile' && (
              <Transformer ref={trRef} boundBoxFunc={(o, n) => (n.width < 5 || n.height < 5) ? o : n} rotateEnabled={false} keepRatio={false} anchorSize={8 / stageScale} borderStrokeWidth={1 / stageScale} anchorStroke={elements.find(el => el.id === selectedIds[0])?.highlightColor || '#3b82f6'} borderStroke={elements.find(el => el.id === selectedIds[0])?.highlightColor || '#3b82f6'} />
            )}
          </Layer>
        </Stage>
        <div className="absolute inset-0 pointer-events-none" style={{ transform: `translate(${stagePos.x}px, ${stagePos.y}px) scale(${stageScale})`, transformOrigin: '0 0' }}>
          {groups.map(g => {
            ...
            return (
              <div key={g.id} className="group" style={{ position: 'absolute', left: g.tileX, top: g.tileY, width: g.tileWidth, height: g.tileHeight, backgroundColor: hexToRgba(g.color, g.opacity ?? 1), border: `2px dashed ${hexToRgba(g.color, (g.opacity ?? 1) * 0.4)}`, borderRadius: 16, pointerEvents: 'none', zIndex: 10 }}>
          ...
          {[...colorTiles, ...tiles].map(el => {
            ...
            return (
              <div key={el.id} className="absolute pointer-events-auto" style={{ left: el.tileX, top: el.tileY, width: elWidth, transform: `scale(${settings.uiScale})`, transformOrigin: 'top left', zIndex: isSelected ? 50 : 20 }}
